...
import static me.qKing12.AuctionMaster.AuctionMaster.blocksCfg;

public class Blocks implements Category{
    ...
    public Blocks(){
        for (String line : blocksCfg.getStringList("custom-item-ids")) {
            priorityIds.add(AuctionMaster.itemConstructor.getItemFromMaterial(line));
        }
        backgroundGlass= AuctionMaster.itemConstructor.getItemFromMaterial("160:"+ AuctionMaster.plugin.getConfig().getString("blocks-menu-color"));
        ItemMeta meta = backgroundGlass.getItemMeta();
        meta.setDisplayName(" ");
        backgroundGlass.setItemMeta(meta);

        displayCategoryItem= AuctionMaster.itemConstructor.getItemFromMaterial(blocksCfg.getString("blocks-menu-item"));
        meta = displayCategoryItem.getItemMeta();
        meta.addItemFlags(ItemFlag.HIDE_ATTRIBUTES, ItemFlag.HIDE_ENCHANTS);
        meta.setDisplayName(Utils.chat(blocksCfg.getString("blocks-menu-name")));
        ArrayList<String> lore = new ArrayList<>();
        for (String line : blocksCfg.getStringList("blocks-menu-lore"))
        ...
        slot= AuctionMaster.menusCfg.getInt("browsing-menu.blocks-slot");
        slot--;
        slot*=9;
    }
    ...
    public boolean checkPriorityName(Auction auction){
        ...
        return blocksCfg.getStringList("custom-item-names").contains(name);
    }
